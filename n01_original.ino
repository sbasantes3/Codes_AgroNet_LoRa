...
void leerSensorSuelo() {
  ...
  bool ok = leerRegistroSuelo(0x0012, v_hum); delay(100);
  if (!ok) { ok_suelo = false; return; }
  leerRegistroSuelo(0x0013, v_temp); delay(100);
  leerRegistroSuelo(0x0015, v_ec);   delay(100);
  leerRegistroSuelo(0x0006, v_ph);   delay(100);
  leerRegistroSuelo(0x001E, v_n);    delay(100);
  leerRegistroSuelo(0x001F, v_p);    delay(100);
  leerRegistroSuelo(0x0020, v_k);    delay(100);
  ok_suelo = true;
...
PaqueteSensor armarPaqueteBinario() {
  ...
  p.hum_suelo  = ok_suelo ? (uint16_t)(suelo_hum*10): INV_U16;
  p.temp_suelo = ok_suelo ? (int16_t)(suelo_temp*10): INV_I16;
  p.ec_suelo   = ok_suelo ? suelo_ec                : INV_U16;
  p.ph_suelo   = ok_suelo ? (uint16_t)(suelo_ph*100): INV_U16;
  p.n_suelo    = ok_suelo ? suelo_n                 : INV_U16;
  p.p_suelo    = ok_suelo ? suelo_p                 : INV_U16;
  p.k_suelo    = ok_suelo ? suelo_k                 : INV_U16;
